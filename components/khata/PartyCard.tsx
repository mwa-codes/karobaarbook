...
import { openWhatsApp, buildPartyBalanceMessage } from "@/lib/whatsapp";
            ...
            {net !== 0 && party.phone ? (
              <button
                ...
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  const msg = buildPartyBalanceMessage(party);
                  openWhatsApp(msg, party.phone ?? undefined);
                }}
                className="mt-1 flex items-center gap-1 rounded-full bg-[#25D366]/10 px-2 py-0.5 text-[#25D366]"
                aria-label="WhatsApp pe message karein"
              >
